...
@app.post("/users/",tags=["Users"],response_model=schemas.Usuariobase)
...
@app.post("/actividades/",tags=["Actividades"],response_model=schemas.Actividadesbase)
...
@app.get("/actividades/", response_model=list[schemas.Actividadesbase],tags=["Actividades"])
def obtener_actividades(db: Session = Depends(get_db)):
    return crud.obtener_actividades(db)

@app.put("/actividades/{actividad_id}", response_model=schemas.Actividadesbase,tags=["Actividades"])
...
@app.delete("/actividades/{actividad_id}",tags=["Actividades"])
